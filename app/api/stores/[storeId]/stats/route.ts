...
export async function GET(_request: NextRequest, context: StoreRouteContext) {
  ...
  try {
    const data = await getProductStats(parsedStoreId.data);

    return NextResponse.json({ data });
  } catch (error) {
    return jsonError(error instanceof Error ? error.message : "Nao foi possivel carregar estatisticas.", 500);
  }
}
